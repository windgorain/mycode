...
static BS_STATUS _webproxy_BuildReqHead(IN _WEB_PROXY_NODE_S *pstNode)
{
    ...
    VBUF_CpyFromBuf(&pstNode->stVBuf, pcHead, ulHeadLen);
    HTTP_DestoryHeadParser(hEncap);
...
static BS_STATUS _webproxy_RecvUpHead(IN FSM_S *pstFsm, IN UINT uiEvent)
{
    ...
    if (VBUF_GetDataLength(&pstNode->stVBuf) != 0)
    {
        if (BS_OK != VBUF_CatFromBuf(&pstNode->stVBuf, aucData, iReadLen))
        {
            return BS_ERR;
        }
    ...
    if (0 == uiHeadLen) {
        ...
        if (bCopyed == FALSE) {
            if (BS_OK != VBUF_CatFromBuf(&pstNode->stVBuf,
                        pucData, uiDataLen)) {
                return BS_ERR;
            }
        }
    ...
    if (VBUF_GetDataLength(&pstNode->stVBuf) == 0)
    {
        if (BS_OK != VBUF_CpyFromBuf(&pstNode->stVBuf, pucData, uiDataLen))
        {
            return BS_ERR;
        }
    }
...
static BS_STATUS _webproxy_RecvUpBody(IN FSM_S *pstFsm, IN UINT uiEvent)
{
    ...
    {
        return VBUF_CatFromBuf(&pstNode->stVBuf, pucData, uiDataLen);
    }
