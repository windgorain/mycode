...
#include "../mybpf_bpf/mybpf_jit_x86_64_inner.h"
...
translate(struct ubpf_vm* vm, struct jit_state* state, char** errmsg)
{
    ...
    for (i = 0; i < vm->num_insts; i++) {
        ...
        case EBPF_OP_LDXW:
            emit_load(state, SIZE_32, src, dst, inst.offset);
            break;
        case EBPF_OP_LDXH:
            emit_load(state, SIZE_16, src, dst, inst.offset);
            break;
        case EBPF_OP_LDXB:
            emit_load(state, SIZE_8, src, dst, inst.offset);
            break;
        case EBPF_OP_LDXDW:
            emit_load(state, SIZE_64, src, dst, inst.offset);
            break;

        case EBPF_OP_STW:
            emit_store_imm32(state, SIZE_32, dst, inst.offset, inst.imm);
            break;
        case EBPF_OP_STH:
            emit_store_imm32(state, SIZE_16, dst, inst.offset, inst.imm);
            break;
        case EBPF_OP_STB:
            emit_store_imm32(state, SIZE_8, dst, inst.offset, inst.imm);
            break;
        case EBPF_OP_STDW:
            emit_store_imm32(state, SIZE_64, dst, inst.offset, inst.imm);
            break;

        case EBPF_OP_STXW:
            emit_store(state, SIZE_32, src, dst, inst.offset);
            break;
        case EBPF_OP_STXH:
            emit_store(state, SIZE_16, src, dst, inst.offset);
            break;
        case EBPF_OP_STXB:
            emit_store(state, SIZE_8, src, dst, inst.offset);
            break;
        case EBPF_OP_STXDW:
