...
* Author:      Xingang.Li  Version: 1.0  Date: 2014-9-30
* Description: 
* History:     
******************************************************************************/
#include "bs.h"
...
#define HTML_ESC_MAX_TMP_LEN 8

typedef struct
{
    HTML_ESC_BUF_S stInput;
    HTML_ESC_BUF_S stOutput;
    UINT uiRefNum;
    CHAR acTmp[HTML_ESC_MAX_TMP_LEN];
    UINT uiTmpLen;
}HTML_ESC_DATA_S;
...
static HTML_ESC_NAME_CHAR_REF_S  g_astHtmlEscDef[] = 
{
  CHAR_REF("AElig", 0xc6),
  CHAR_REF("AMP;", 0x26),
  CHAR_REF("AMP", 0x26),
...
static VOID html_esc_ProcessRefName(IN DFA_HANDLE hDfa);
static VOID html_esc_InitRef(IN DFA_HANDLE hDfa);
static VOID html_esc_AddThis2Output(IN DFA_HANDLE hDfa);
static VOID html_esc_AddTmp(IN DFA_HANDLE hDfa);
static VOID html_esc_ProcessNum10(IN DFA_HANDLE hDfa);
static VOID html_esc_ProcessNum16(IN DFA_HANDLE hDfa);
static VOID html_esc_RefEnd(IN DFA_HANDLE hDfa);
static VOID html_esc_AddTmp2Output(IN DFA_HANDLE hDfa);

static DFA_NODE_S g_astHtmlEscStateData[] = 
{
    DFA_NODE( DFA_CODE_CHAR('&'), HTML_ESC_REF_OPEN, "init" ),
    DFA_NODE( DFA_CODE_END, HTML_ESC_DATA, NULL ),
    DFA_NODE( DFA_CODE_OTHER, HTML_ESC_DATA, "output" )
};

static DFA_NODE_S g_astHtmlEscStateRefOpen[] = 
{
    DFA_NODE( DFA_CODE_END, HTML_ESC_DATA, "outputTmp" ),
    DFA_NODE( DFA_CODE_CHAR('&'), HTML_ESC_REF_OPEN, "outputTmp,init" ),
    DFA_NODE( DFA_CODE_CHAR('#'), HTML_ESC_REF_NUM, "tmp" ),
    DFA_NODE( DFA_CODE_ALPHA, HTML_ESC_REF_NAME, "name" ),
    DFA_NODE( DFA_CODE_OTHER, HTML_ESC_DATA, "outputTmp,output" )
};

static DFA_NODE_S g_astHtmlEscStateRefNum[] = 
{
    DFA_NODE( DFA_CODE_END, HTML_ESC_DATA, "outputTmp" ),
    DFA_NODE( DFA_CODE_CHAR('&'), HTML_ESC_REF_OPEN, "outputTmp,init" ),
    DFA_NODE( DFA_CODE_CHAR('x'), HTML_ESC_REF_NUM_PRE_16, "tmp" ),
    DFA_NODE( DFA_CODE_CHAR('X'), HTML_ESC_REF_NUM_PRE_16, "tmp" ),
    DFA_NODE( DFA_CODE_NUMBER, HTML_ESC_REF_NUM_10, "num10" ),
    DFA_NODE( DFA_CODE_OTHER, HTML_ESC_DATA, "outputTmp,output" )
};

static DFA_NODE_S g_astHtmlEscStateRefNumPre16[] = 
{
    DFA_NODE( DFA_CODE_END, HTML_ESC_DATA, "outputTmp" ),
    DFA_NODE( DFA_CODE_HEX, HTML_ESC_REF_NUM_16, "num16" ),
    DFA_NODE( DFA_CODE_CHAR('&'), HTML_ESC_REF_OPEN, "outputTmp,init" ),
    DFA_NODE( DFA_CODE_OTHER, HTML_ESC_DATA, "outputTmp,output" )
};

static DFA_NODE_S g_astHtmlEscStateRefNum16[] = 
{
    DFA_NODE( DFA_CODE_END, HTML_ESC_DATA, "end" ),
    DFA_NODE( DFA_CODE_CHAR(';'), HTML_ESC_DATA, "end" ),
    DFA_NODE( DFA_CODE_CHAR('&'), HTML_ESC_REF_OPEN, "end,init" ),
    DFA_NODE( DFA_CODE_HEX, HTML_ESC_REF_NUM_16, "num16" ),
    DFA_NODE( DFA_CODE_OTHER, HTML_ESC_DATA, "end,output" )
};

static DFA_NODE_S g_astHtmlEscStateRefNum10[] = 
{
    DFA_NODE( DFA_CODE_END, HTML_ESC_DATA, "end" ),
    DFA_NODE( DFA_CODE_CHAR(';'), HTML_ESC_DATA, "end" ),
    DFA_NODE( DFA_CODE_CHAR('&'), HTML_ESC_REF_OPEN, "end,init" ),
...
static DFA_NODE_S g_astHtmlEscStateRefName[] = 
{
    DFA_NODE( DFA_CODE_CHAR(';'), HTML_ESC_DATA, NULL ),
    DFA_NODE( DFA_CODE_OTHER, HTML_ESC_REF_NAME, NULL )
};

static DFA_TBL_LINE_S g_astHtmlEscDfaTable[] =
{
    DFA_TBL_LINE(g_astHtmlEscStateData),
    DFA_TBL_LINE(g_astHtmlEscStateRefOpen),
    DFA_TBL_LINE(g_astHtmlEscStateRefNum),
    DFA_TBL_LINE(g_astHtmlEscStateRefNumPre16),
    DFA_TBL_LINE(g_astHtmlEscStateRefNum16),
    DFA_TBL_LINE(g_astHtmlEscStateRefNum10),
    DFA_TBL_LINE(g_astHtmlEscStateRefName),
    DFA_TBL_END
};

static ACTION_S g_astHtmlEscActions[] =
{
    ACTION_LINE("init", html_esc_InitRef),
    ACTION_LINE("tmp", html_esc_AddTmp),
    ACTION_LINE("output", html_esc_AddThis2Output),
    ACTION_LINE("outputTmp", html_esc_AddTmp2Output),
    ACTION_LINE("name", html_esc_ProcessRefName),
    ACTION_LINE("num10", html_esc_ProcessNum10),
    ACTION_LINE("num16", html_esc_ProcessNum16),
    ACTION_LINE("end", html_esc_RefEnd),
    ACTION_END
};


static VOID html_esc_Consume(IN HTML_ESC_BUF_S *pstBuf, IN UINT uiConsumeLen)
{
    BS_DBGASSERT(pstBuf->uiBufLen >= uiConsumeLen);

    pstBuf->pcBuf += uiConsumeLen;
    pstBuf->uiBufLen -= uiConsumeLen;
}

static HTML_ESC_NAME_CHAR_REF_S * html_esc_FindNamedCharRef(IN const HTML_ESC_BUF_S *pstBuf)
{
    UINT uiIndex;
    HTML_ESC_NAME_CHAR_REF_S *pstCurrent;
    HTML_ESC_NAME_CHAR_REF_S *pstFound = NULL;

    for (uiIndex = 0; g_astHtmlEscDef[uiIndex].stCodePoints.iFirst != -1; uiIndex++)
    {
        pstCurrent = &g_astHtmlEscDef[uiIndex];
        if ((pstBuf->uiBufLen >= pstCurrent->uiNameLen)
            && (0 == memcmp(pstBuf->pcBuf, pstCurrent->pcName, (ULONG)pstCurrent->uiNameLen)))
        {
            pstFound = pstCurrent;
            break;
        }
...
static VOID html_esc_AppendChar(IN HTML_ESC_BUF_S *pstBuf, IN UCHAR ucChar)
{
    if (pstBuf->uiBufLen > 0)
    {
        pstBuf->pcBuf[0] = (CHAR)ucChar;
        pstBuf->pcBuf ++;
        pstBuf->uiBufLen --;
    }
}
...
static VOID html_esc_AppendPoint(IN HTML_ESC_DATA_S *pstData, IN UINT uiData)
{
    ...
    else if (uiData <= 0x7ff)
    {
        uiBytesNum = 1;
        uiPrefix = 0xc0;
    }
    else if (uiData <= 0xffff)
    {
        uiBytesNum = 2;
        uiPrefix = 0xe0;
    }
    ...
    html_esc_AppendChar(&pstData->stOutput, (UCHAR)(uiPrefix | (uiData >> (uiBytesNum * 6))));

    for (iIndex = (INT)uiBytesNum - 1; iIndex >= 0; iIndex--)
    {
        html_esc_AppendChar(&pstData->stOutput, (UCHAR)(0x80 | (0x3f & (uiData >> (iIndex * 6)))));
    }
}

static VOID html_esc_InitRef(IN DFA_HANDLE hDfa)
{
    HTML_ESC_DATA_S *pstData;

    pstData = DFA_GetUserData(hDfa);
...
static VOID html_esc_AddTmp(IN DFA_HANDLE hDfa)
{
    ...
    pstData->acTmp[pstData->uiTmpLen] = (CHAR)(UCHAR)DFA_GetInputCode(hDfa);
    pstData->uiTmpLen ++;
}
...
static VOID html_esc_AddThis2Output(IN DFA_HANDLE hDfa)
{
    ...
    html_esc_AppendChar(&pstData->stOutput, (CHAR)(UCHAR)DFA_GetInputCode(hDfa));
}
...
static VOID html_esc_RefEnd(IN DFA_HANDLE hDfa)
{
    ...
    html_esc_AppendPoint(pstData, uiNum);
}
...
static VOID html_esc_AddTmp2Output(IN DFA_HANDLE hDfa)
{
    ...
    for (uiIndex=0; uiIndex<pstData->uiTmpLen; uiIndex++)
    {
        html_esc_AppendChar(&pstData->stOutput, (UCHAR)pstData->acTmp[uiIndex]);
    }
...
static VOID html_esc_ProcessNum10(IN DFA_HANDLE hDfa)
{
    ...
    uiNum = (UCHAR)DFA_GetInputCode(hDfa) - (UCHAR)'0';

    pstData->uiRefNum = pstData->uiRefNum * 10 + uiNum;
}
...
static VOID html_esc_ProcessNum16(IN DFA_HANDLE hDfa)
{
    ...
    uiNum = (UINT)HEX_2_NUM((CHAR)(UCHAR)DFA_GetInputCode(hDfa));

    pstData->uiRefNum = pstData->uiRefNum * 16 + uiNum;
}

static VOID html_esc_ProcessRefName(IN DFA_HANDLE hDfa)
{
    HTML_ESC_DATA_S *pstData;
    HTML_ESC_NAME_CHAR_REF_S *pstFound;

    pstData = DFA_GetUserData(hDfa);

    pstFound = html_esc_FindNamedCharRef(&pstData->stInput);
    if (NULL == pstFound)
    {
        html_esc_AppendChar(&pstData->stOutput, (UCHAR)'&');
        html_esc_AppendChar(&pstData->stOutput, (CHAR)(UCHAR)DFA_GetInputCode(hDfa));
    }
    else
    {
        if (pstFound->stCodePoints.iFirst != -1)
        {
            html_esc_AppendPoint(pstData, (UINT)pstFound->stCodePoints.iFirst);
        }

        if (pstFound->stCodePoints.iSecond!= -1)
        {
            html_esc_AppendPoint(pstData, (UINT)pstFound->stCodePoints.iSecond);
        }

        html_esc_Consume(&pstData->stInput, pstFound->uiNameLen - 1);
    }

    DFA_SetState(hDfa, HTML_ESC_DATA);
}

static void html_esc_Init()
{
    ...
    if (bInit == FALSE)
    {
        ...
        DFA_Compile(g_astHtmlEscDfaTable, g_astHtmlEscActions);
    }
}
...
{
    HTML_ESC_DATA_S stData;
    DFA_HANDLE hDfa;

    html_esc_Init();

    hDfa = DFA_Create(g_astHtmlEscDfaTable, g_astHtmlEscActions, HTML_ESC_DATA);
    ...
    DFA_SetUserData(hDfa, &stData);

    memset(&stData, 0, sizeof(stData));

    stData.stInput.pcBuf = pcInput;
    stData.stInput.uiBufLen = uiInputLen;
    stData.stOutput.pcBuf = pcOutput;
    stData.stOutput.uiBufLen = uiOutBufSize - 1;

    while (stData.stInput.uiBufLen > 0)
    {
        DFA_InputChar(hDfa, stData.stInput.pcBuf[0]);
        html_esc_Consume(&stData.stInput, 1);
    }
    ...
    stData.stOutput.pcBuf[0] = '\0';

    return BS_OK;
}

static VOID html_esc_Translate(IN HTML_ESC_BUF_S *pstOutput, IN UCHAR ucInputCode)
{
    CHAR szRefNum[16];
    CHAR *pcTmp;

    snprintf(szRefNum, sizeof(szRefNum), "&#%d;", ucInputCode);

    pcTmp = szRefNum;
    while (*pcTmp != '\0')
    {
        html_esc_AppendChar(pstOutput, (UCHAR)(*pcTmp));
        pcTmp ++;
    }
}
...
)
{
    ...
    {
        if (strchr(pcTranslateCharset, pcInput[uiIndex]) != 0)
        {
            uiNeedLen += (sizeof("&#255;") - 1);
        }
        else
        {
            uiNeedLen ++;
...
)
{
    ...
    stOutput.pcBuf = pcOutput;
    stOutput.uiBufLen = uiOutBufSize - 1;

    for (uiIndex=0; uiIndex<uiInputLen; uiIndex++)
    {
        if (strchr(pcTranslateCharset, pcInput[uiIndex]) != 0)
        {
            html_esc_Translate(&stOutput, (UCHAR)pcInput[uiIndex]);
        }
        else
        {
            html_esc_AppendChar(&stOutput, (UCHAR)pcInput[uiIndex]);
        }
    }

    stOutput.pcBuf[0] = '\0';

    return (UINT)((ULONG)stOutput.pcBuf - (ULONG)pcOutput);
}
