...
#define MYBPF_JIT_STACK_SIZE 512
...
typedef struct {
    ...
    int max_num_calls;
    uint32_t stack_size;
    uint32_t save_ext_agent: 1;
    uint32_t call_ext_agent: 1;
}MYBPF_JIT_STATE_S;
...
static enum Registers callee_saved_registers[] = {R19, R20, R21, R22, R23, R24, R25, R26, R27, R28};
...
/* 用于保存agent */
static enum Registers agent_register = R27;
static enum Registers ud_register = R28;
...
static void emit_movewide_immediate(MYBPF_JIT_STATE_S *state, bool sixty_four, enum Registers rd, uint64_t imm);
static void divmod(MYBPF_JIT_STATE_S *state, uint16_t pc, uint8_t opcode, int rd, int rn, int rm);

static void
emit_bytes(MYBPF_JIT_STATE_S *state, void *data, uint32_t len)
{
...
static void emit_instruction(MYBPF_JIT_STATE_S *state, uint32_t instr)
{
...
static void
emit_addsub_immediate(MYBPF_JIT_STATE_S *state, bool sixty_four, enum AddSubOpcode op, enum Registers rd, enum Registers rn, uint32_t imm12)
{
...
static void
emit_addsub_register(MYBPF_JIT_STATE_S *state, bool sixty_four, enum AddSubOpcode op, enum Registers rd, enum Registers rn, enum Registers rm)
{
...
static void
emit_loadstore_immediate(MYBPF_JIT_STATE_S *state, enum LoadStoreOpcode op, enum Registers rt, enum Registers rn, int16_t imm9)
{
...
static void
emit_loadstore_register(MYBPF_JIT_STATE_S *state, enum LoadStoreOpcode op, enum Registers rt, enum Registers rn, enum Registers rm)
{
...
static void
emit_loadstorepair_immediate(MYBPF_JIT_STATE_S *state, enum LoadStorePairOpcode op, enum Registers rt, enum Registers rt2, enum Registers rn, int32_t imm7)
{
...
static void
emit_logical_register(MYBPF_JIT_STATE_S *state, bool sixty_four, enum LogicalOpcode op, enum Registers rd, enum Registers rn, enum Registers rm)
{
...
static void
emit_unconditonalbranch_register(MYBPF_JIT_STATE_S *state, enum UnconditionalBranchOpcode op, enum Registers rn)
{
...
static void
note_jump(MYBPF_JIT_STATE_S *state, uint32_t target_pc)
{
    if (state->num_jumps == MYBPF_JIT_MAX_INSTS) {
        return;
    }
    MYBPF_JIT_JUMP_S *jump = &state->jumps[state->num_jumps++];
...
static void
emit_unconditionalbranch_immediate(MYBPF_JIT_STATE_S *state, enum UnconditionalBranchImmediateOpcode op, int32_t target_pc)
{
...
static void
emit_conditionalbranch_immediate(MYBPF_JIT_STATE_S *state, enum Condition cond, uint32_t target_pc)
{
    note_jump(state, target_pc);
...
static void
emit_dataprocessing_onesource(MYBPF_JIT_STATE_S *state, bool sixty_four, enum DP1Opcode op, enum Registers rd, enum Registers rn)
{
...
static void
emit_dataprocessing_twosource(MYBPF_JIT_STATE_S *state, bool sixty_four, enum DP2Opcode op, enum Registers rd, enum Registers rn, enum Registers rm)
{
...
static void
emit_dataprocessing_threesource(MYBPF_JIT_STATE_S *state, bool sixty_four, enum DP3Opcode op, enum Registers rd, enum Registers rn, enum Registers rm, enum Registers ra)
{
...
/* 未经优化的32位赋值 */
static inline void emit_move_imm32_unoptimized(MYBPF_JIT_STATE_S *state, enum Registers rd, uint32_t imm)
{
    uint32_t imm16 = (imm & 0xffff);
    emit_instruction(state, 0 | MW_MOVZ | 0 | (imm16 << 5) | rd);
    imm16 = (imm >> 16) & 0xffff;
    emit_instruction(state, 0 | MW_MOVK | (1 << 21) | (imm16 << 5) | rd);
}

/* 未经优化的64位赋值 */
static inline void emit_move_imm64_unoptimized(MYBPF_JIT_STATE_S *state, enum Registers rd, uint64_t imm)
{
    unsigned int i;
    u64 imm16 = (imm & 0xffff);

    emit_instruction(state, (1 << 31) | MW_MOVZ | 0 | (imm16 << 5) | rd);
    for (i=1; i<4; i++) {
        imm16 = (imm >> (i * 16)) & 0xffff;
        emit_instruction(state, (1 << 31) | MW_MOVK | (i << 21) | (imm16 << 5) | rd);
    }
}

/* [ArmARM-A H.a]: C4.1.64: Move wide (Immediate).  */
static void
emit_movewide_immediate(MYBPF_JIT_STATE_S *state, bool sixty_four, enum Registers rd, uint64_t imm)
{
...
static void update_branch_immediate(MYBPF_JIT_STATE_S *state, uint32_t offset, int32_t imm)
{
    assert((imm & 3) == 0);
    uint32_t instr;
    imm >>= 2;
 ...
 *   SP on entry
 *   ubpf_stack_size bytes of BPF stack
 *   Callee saved registers
 *   Frame <- SP.
 */
static void
emit_function_prologue(MYBPF_JIT_STATE_S *state, size_t ubpf_stack_size)
{
    uint32_t register_space =  ARRAY_SIZE(callee_saved_registers) * 8 + 2 * 8;
    /* 计算需要的栈空间大小并16字节对齐 */
    state->stack_size = (ubpf_stack_size + register_space + 15) & ~15U;
    emit_addsub_immediate(state, true, AS_SUB, SP, SP, state->stack_size);

    /* Set up frame */
    /* 把R29,R30 存入SP[0],SP[1] */
    emit_loadstorepair_immediate(state, LSP_STPX, R29, R30, SP, 0);
    emit_addsub_immediate(state, true, AS_ADD, R29, SP, 0);

    /* Save callee saved registers */
    unsigned i;
    int size = ARRAY_SIZE(callee_saved_registers);

    if (! state->save_ext_agent) {
        /* 不需要保存agent_register */
        size -= 2;
    }
    ...
    /* Setup BPF frame pointer. */
    emit_addsub_immediate(state, true, AS_ADD, map_register(10), SP, state->stack_size);

    /* 保存agent func, 第6个入参(R5)存放的就是agent func */
    if (state->save_ext_agent) {
        emit_addsub_register(state, true, AS_SUB, agent_register, R5, 0);
        emit_addsub_register(state, true, AS_SUB, ud_register, R6, 0);
    }
}

static void emit_call(MYBPF_JIT_STATE_S *state, uintptr_t func) {
...
/* 通过代理进行call, 第6和7个参数分别是func_id和ud */
static void emit_call_agent(MYBPF_JIT_STATE_S *state, unsigned int func_id) {
    emit_addsub_register(state, true, AS_SUB, R6, ud_register, 0);
    emit_movewide_immediate(state, false, R5, func_id);
    emit_unconditonalbranch_register(state, BR_BLR, agent_register);
...
/* bpf2bpf调用 */
static void emit_call_bpf(MYBPF_JIT_STATE_S *state, uintptr_t func) {
    emit_instruction(state, UBR_BL | (func & 0x3ffffff));
...
static void emit_function_epilogue(MYBPF_JIT_STATE_S *state)
{
    ...
    for (i = 0; i < size; i += 2) {
        emit_loadstorepair_immediate(state, LSP_LDPX, callee_saved_registers[i],
                callee_saved_registers[i + 1], SP, (i + 2) * 8);
    }
...
static bool
is_imm_op(const MYBPF_INSN_S * inst)
{
...
static bool
is_alu64_op(const MYBPF_INSN_S * inst)
{
    int class = inst->opcode & EBPF_CLS_MASK;
...
static bool
is_simple_imm(const MYBPF_INSN_S *inst)
{
    switch (inst->opcode) {
...
static int
translate(MYBPF_JIT_VM_S *vm, MYBPF_JIT_CTX_S *ctx, MYBPF_JIT_STATE_S *state)
{
    int i;
    uintptr_t raw_func;

    emit_function_prologue(state, ctx->stack_size);

    for (i = 0; i < vm->num_insts; i++) {
        MYBPF_INSN_S inst = vm->insts[i];
        ...
        case EBPF_OP_CALL:
            if (inst.src_reg == BPF_PSEUDO_CALL) {
                /* 先填写成imm, 之后再修正 */
                emit_call_bpf(state, inst.imm);
            } else {
                if (state->call_ext_agent){
                    emit_call_agent(state, inst.imm);
                } else {
                    raw_func = vm->base_func_addr + inst.imm;
                    emit_call(state, raw_func);
                }
                if (inst.imm == vm->tail_call_func) {
        ...
        case EBPF_OP_RSH64_IMM:
        case EBPF_OP_ARSH64_IMM:
            RETURNI(BS_ERR, "Unexpected instruction at PC %d: opcode 0x%02x, immediate 0x%08x",
                    i, opcode, inst.imm);
            return -1;
        default:
            RETURNI(BS_ERR, "Unknown instruction at PC %d: opcode 0x%02x", i, opcode);
            return -1;
        }
    }
    ...
    /* 调用 report_err(err_code, pc); */
    emit_movewide_immediate(state, false, caller_saved_registers[0], MYBPF_EXCEPTION_CODE_DIV_0);
    emit_call(state, MYBPF_JIT_EXCEPTION_FUNC_ID);
    /* 退出, ret -1 */
...
static void
divmod(MYBPF_JIT_STATE_S *state, uint16_t pc, uint8_t opcode, int rd, int rn, int rm)
{
...
static void
resolve_jumps(MYBPF_JIT_STATE_S *state)
{
    for (unsigned i = 0; i < state->num_jumps; ++i) {
        MYBPF_JIT_JUMP_S jump = state->jumps[i];
...
int MYBPF_JitArm64_Jit(MYBPF_JIT_VM_S *vm, OUT MYBPF_JIT_CTX_S *ctx)
{
    ...
    state.size = ctx->max_jitted_size;
    state.buf = ctx->jitted_buf;
    state.save_ext_agent = ctx->save_ext_agent;
    state.call_ext_agent = ctx->call_ext_agent;

    state.pc_locs = ctx->locs;
    if (! state.pc_locs) {
...
{
    int i;
    MYBPF_INSN_CALLS_S *c;
    UINT *to_fix;
    int cur_pc;
    int dst;

    for (i=0; i<res->calls_count; i++) {
        c = &res->calls[i];
        ...
        /* 获取call bpf对应的jitted_buf中的位置 */
        to_fix = (void*)((char*)res->jitted_buf + res->locs[c->insn_index]);

        /* 获取当前指令偏移 */
        cur_pc = res->locs[c->insn_index];
        /* 获取目标insn_index对应的偏移,再减去函数头的长度即为入口 */
        dst = res->locs[c->imm + c->insn_index] - res->locs[0];
        /* 计算相对偏移, 以4字节为一个单位 */
        int off = (dst/4 - cur_pc/4) & 0x3ffffff;
