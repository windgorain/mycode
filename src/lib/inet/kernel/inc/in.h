...
struct ipovly {
    UCHAR    ih_x1[9];        /* (unused) */
    UCHAR    ih_pr;            /* protocol */
    USHORT    ih_len;            /* protocol length */
    UINT ih_src;        /* source internet address */
    UINT ih_dst;        /* destination internet address */
};
...
typedef struct snd_if {
    ...
    UINT si_nxthop;   /* nexthop address */
    UINT si_lcladdr;  /* local address */
}SND_IF_S;
