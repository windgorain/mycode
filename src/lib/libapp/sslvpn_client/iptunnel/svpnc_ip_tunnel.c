...
static BS_WALK_RET_E _svpnc_iptunnel_UserEvent(IN UINT uiTriggerEvent, IN USER_HANDLE_S *pstUserHandle)
{
    if (g_stSvpncIpTunnel.pstMbufSendding != NULL)
    {
        return BS_WALK_CONTINUE;    /* 上一个报文没有发送完成,无需发送下一个报文 */
    }
...
static BS_WALK_RET_E _svpnc_iptunnel_ConnEventNotify
(
    IN INT iSocketId,
    IN UINT uiEvent,
