...
static BS_STATUS tun_IfCreate(IF_INDEX ifIndex)
{
    ...
    CompIf_SetUserData(ifIndex, g_uiTunIfUserDataIndex, UINT_HANDLE(i));
...
)
{
    ...
    if (BS_OK != CompIf_GetUserData(ifIndex, g_uiTunIfUserDataIndex, (HANDLE*)&hId)) {
        MBUF_Free(pstMbuf);
        return BS_OK;
    }
...
static BS_STATUS tun_RegIf()
{
    ...
    CompIf_SetLinkType("link.tun", &stLinkParam);
    ...
    g_uiTunIfType = CompIf_AddIfType("if.tun", &stTypeParam);

    g_uiTunIfUserDataIndex = CompIf_AllocUserDataIndex();
...
int TUN_LinkInput(int id, MBUF_S *mbuf)
{
    ...
    return CompIf_LinkInput(g_tun_ifs[id].ifindex, mbuf);
}
