...
static BS_STATUS _aifpcap_PhyOutput(IN UINT uiIfIndex, IN MBUF_S *pstMbuf)
{
    ...
    if ((BS_OK != IFNET_GetUserData(uiIfIndex, g_uiAifPcapIfUserDataIndex, (HANDLE*)&pstCtrl))
...
static BS_STATUS _aifpcap_PhyIoctl(IN IF_INDEX ifIndex, IN UINT uiCmd/* IF_PHY_IOCTL_CMD_E */, INOUT VOID *pData)
{
    ...
    if ((BS_OK != IFNET_GetUserData(ifIndex, g_uiAifPcapIfUserDataIndex, (HANDLE*)&pstCtrl))
        || (NULL == pstCtrl) || (pstCtrl->uiUsePcapIndex == PCAP_INVALID_INDEX))
    {
        return BS_ERR;
    }
    ...
    switch(uiCmd)
    {
        ...
        {
            pstCtrl->shutdown = HANDLE_UINT(pData);
            break;
        }
        case IF_PHY_IOCTL_CMD_GET_PRIVATE_INFO:
        {
            eRet = PCAP_Ioctl(pstCtrl->uiUsePcapIndex, PCAP_IOCTL_GET_PRIVATE_INFO, pData);
            break;
        }
...
)
{
    ...
    IFNET_LinkInput(pstPcapCtrl->ifIndex, pstMbuf);
}
        ...
        IN _AIF_PCAP_CTRL_S *pstCtrl, IN UINT uiPcapIndex)
{
    ...
    PCAP_RegService(uiPcapIndex, ETH_P_IP, _aifpcap_PhyPktIn, &stUserHandle);
    PCAP_RegService(uiPcapIndex, ETH_P_ARP, _aifpcap_PhyPktIn, &stUserHandle);
...
static BS_STATUS _aifpcap_IfCreate(IN IF_INDEX ifIndex)
{
    ...
    if (BS_OK != PCAP_Ioctl(uiPcapIndex, PCAP_IOCTL_CMD_GET_NET_INFO,
                &stInfo)) {
        return BS_ERR;
    }

    IFNET_Ioctl(ifIndex, IFNET_CMD_SET_MAC, &stInfo.stMacAddr);

    pstPcap->ifIndex = ifIndex;

    IFNET_SetUserData(ifIndex, g_uiAifPcapIfUserDataIndex, pstPcap);
...
static VOID _aifpcap_IfDelete(IN IF_INDEX ifIndex)
{
    ...
    if ((BS_OK != IFNET_GetUserData(ifIndex, g_uiAifPcapIfUserDataIndex,
                    (HANDLE*)&pstPcap)) || (NULL == pstPcap)) {
        return;
    }

    IFNET_SetUserData(ifIndex, g_uiAifPcapIfUserDataIndex, NULL);
...
static BS_STATUS _aifpcap_RegPcapIf()
{
    ...
    IFNET_SetPhyType(IF_PCAP_PHY_TYPE_MAME, &stPhyParam);
    ...
    g_uiAifPcapIfType = IFNET_AddIfType(IF_PCAP_IF_TYPE_MAME, &stTypeParam);
...
static IF_INDEX _aifpcap_AutoCratePcapIf(IN UINT uiPcapIndex, IN CHAR *pcIfName)
{
    ...
    ifIndex = IFNET_CreateIfByName(IF_PCAP_IF_TYPE_MAME, pcIfName);
...
{
    _aifpcap_RegPcapIf();
    g_uiAifPcapIfUserDataIndex = IFNET_AllocUserDataIndex();
...
BS_STATUS AIF_PCAP_AutoCreateInterface()
{
    ...
    {
        if (BS_OK != PCAP_Ioctl(i, PCAP_IOCTL_CMD_GET_NAME, szAdapterName))
        {
            continue;
        }
        ...
        ifIndex = IFNET_GetIfIndex(szIfName);
        if (ifIndex == IF_INVALID_INDEX)
        {
