...
struct lpm {
    /* LPM metadata. */
    char name[LPM_NAMESIZE];        /**< Name of the lpm. */
    uint32_t max_rules; /**< Max. balanced rules per lpm. */
    uint32_t number_tbl8s; /**< Number of tbl8s. */
    struct lpm_rule_info rule_info[LPM_MAX_DEPTH]; /**< Rule info table. */

    /* LPM Tables. */
    struct lpm_tbl_entry tbl24[LPM_TBL24_NUM_ENTRIES];
    struct lpm_tbl_entry *tbl8; /**< LPM tbl8 table. */
    struct lpm_rule *rules_tbl; /**< LPM rules. */
};
 ...
 *
 * @param name
 *   Name of the lpm object as passed to lpm_create()
 * @return
 ...
 *   LPM_LOOKUP_SUCCESS is set). The least significant byte is the
 *   actual next hop.
 * @param n
