...
static int _klcko_regkp(void)
{
    ...
    kp.symbol_name = "do_check";
    kp.pre_handler = _klcko_handler_pre;
    kp.post_handler = _klcko_handler_post;
    kp.fault_handler = _klcko_handler_fault;
    ...
    memset(&kp, 0, sizeof(kp));
    kp.symbol_name = "do_check_common";
    kp.pre_handler = _klcko_handler_pre;
    kp.post_handler = _klcko_handler_post;
    kp.fault_handler = _klcko_handler_fault;
    ...
    ret = register_kprobe(&kp);
    if (ret < 0) {
        printk(KERN_INFO "register failed, returned %d\n", ret);
        return ret;
    }
...
static inline u64 _klcko_do_base(u64 type, u64 p2, u64 p3, u64 p4, u64 p5)
{
    switch (type) {
        ...
        case KLCHELP_GET_NAME_MAP:
            return (unsigned long)(long)KlcKoNameMap_BpfGet((void*)p2);
...
static inline u64 _klcko_do_mem(u64 type, u64 p2, u64 p3, u64 p4, u64 p5)
{
    switch (type) {
...
static inline u64 _klcko_do_string(u64 type, u64 p2, u64 p3, u64 p4, u64 p5)
{
    switch (type) {
        case KLCHELP_STRLEN:
...
static inline u64 _klcko_do_skb(u64 type, u64 p2, u64 p3, u64 p4, u64 p5)
{
    switch(type) {
...
static inline u64 _klcko_do(u64 type, u64 p2, u64 p3, u64 p4, u64 p5)
{
    int class = KLCHELP_CLASS(type);

    switch (class) {
        case KLCHELP_CLASS_BASE:
            return _klcko_do_base(type, p2, p3, p4, p5);
        case KLCHELP_CLASS_MEM:
            return _klcko_do_mem(type, p2, p3, p4, p5);
        case KLCHELP_CLASS_STRING:
            return _klcko_do_string(type, p2, p3, p4, p5);
        case KLCHELP_CLASS_SKB:
            return _klcko_do_skb(type, p2, p3, p4, p5);
        default:
            return KLC_RET_ERR;
    }
...
static inline u64 _klcko_internal_do(u64 type, u64 p2, u64 p3, u64 p4, u64 p5)
{
    switch (type) {
...
u64 KlcKo_Do(u64 type, u64 p2, u64 p3, u64 p4, u64 p5)
{
    static DEFINE_PER_CPU(int, call_count); /* 禁止调用深度超过32 */
    u64 ret;

    if (this_cpu_read(call_count) >= 32) {
        return KLC_RET_ERR;
    }

    this_cpu_inc(call_count);
    ret = _klcko_do(type, p2, p3, p4, p5);
    this_cpu_dec(call_count);

    return ret;
}

u64 KlcKo_InternalDo(u64 type, u64 p2, u64 p3, u64 p4, u64 p5)
{
    return _klcko_internal_do(type, p2, p3, p4, p5);
}
