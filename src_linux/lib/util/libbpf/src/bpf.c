		   ...
		   const struct bpf_map_create_opts *opts)
{
    ...
    if (UNAME_GetKernelVersion() > KERNEL_VERSION(4, 19, 0)) {
...
int bpf_obj_get_attr_by_fd(int bpf_fd,union bpf_attr *attr, int attr_size)
{
    int err;

	err = sys_bpf(BPF_OBJ_GET_INFO_BY_FD, attr, attr_size);
