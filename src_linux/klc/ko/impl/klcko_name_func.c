...
static int klcko_func_nlget_name(NETLINK_MSG_S *msg, KLC_FUNC_S *func)
{
    ...
    n = klcko_func_find_name(func->hdr.name);

    return KO_NL_GetOne(msg, n, sizeof(KLC_FUNC_S));
}
...
static int klcko_name_func_init(KLC_FUNC_INIT_S *init)
{
    ...
    if (KLC_RET_ERR == KlcKo_NameLoadRun(init->func, 0, 0, 0)) {
        return KO_ERR_FAIL;
    }
