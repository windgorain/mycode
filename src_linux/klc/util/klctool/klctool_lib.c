...
static unsigned char g_klctool_insn_hdr[] = { 0x01,0x01,0x00,0x00,0x00,0x00,0x00,0x00 };
...
static KLC_FUNC_S * _klctool_alloc_idname_func(void *data, int data_len,
        int private_data_size, char *module_name, char *name)
{
    KLC_FUNC_S *func = NULL;
    int len;

    len = sizeof(KLC_FUNC_S) + data_len + KLC_BPF_HEADER_SIZE + private_data_size;
    ...
    func->hdr.size = len;
    func->private_data_size = private_data_size;
...
static int _klctool_add_func(void *nl, void *data, int data_len, int private_data_size, char *module_name,
        int type, int id, char *name, int replace)
{
    KLC_FUNC_S *func = NULL;
    int nlcmd;
    ...
    func = _klctool_alloc_idname_func(data, data_len, private_data_size, module_name, name);
    if (! func) {
        return -1;
    }
...
static int _klctool_load_id_func_one(KLCTOOL_LOAD_INFO_S *loadinfo, int type, int id, char *name)
{
    ...
    KLC_FUNC_ATTR_S * attr = _klctool_get_func_attr(loadinfo->elf, name);
    if (attr) {
        private_data_size = attr->private_data_size;
    }

    return _klctool_add_func(loadinfo->nl, sec->data->d_buf, sec->data->d_size, private_data_size,
...
static int _klctool_load_hashmap_func(KLCTOOL_LOAD_INFO_S *loadinfo)
{
    ...
    if (loadinfo->cmd == KLC_NL_OBJ_LOAD) {
        ...
        if (code) {
            ...
            memcpy(&code[sizeof(g_klctool_insn_hdr)], sec->data->d_buf, sec->data->d_size);
...
static int _klctool_unload_subs(ELF_S *elf, KLCTOOL_LOAD_INFO_S *loadinfo)
{
    ELF_SECTION_S sec;
    void *iter = NULL;
    char buf[256];
...
static int _klctool_load_subs(ELF_S *elf, KLCTOOL_LOAD_INFO_S *loadinfo)
{
    void *iter = NULL;
    ELF_SECTION_S sec;
    char buf[256];
...
static int _klctool_load_by_file(NETLINK_S *nl, int cmd, KLCTOOL_LOAD_S *load, ELF_S *elf)
{
    ...
    if (cmd == KLC_NL_OBJ_UNLOAD) {
        return _klctool_unload_subs(elf, &loadinfo);
    }

    return _klctool_load_subs(elf, &loadinfo);
}
...
static void _klctool_func_show_info(KUTL_NL_GET_S *show)
{
    ...
    for (i=0; i<show->count; i++) {
        if (func->type == KLC_FUNC_TYPE_ID) {
            printf(" name:%s, id:%u, len:%u \n",
                    func->hdr.name, func->id, func->insn_len);
        } else {
            printf(" name:%s, len:%u \n", func->hdr.name, func->insn_len);
        }
        func ++;
    }
}
...
static int _klctool_evob_show(NETLINK_S *nl, int cmd, void *data, int data_len)
{
    KUTL_NL_GET_S *show;
    int ret;
...
static int _klctool_map_show(NETLINK_S *nl, int cmd, void *data, int data_len)
{
    KUTL_NL_GET_S *show;
    int ret;
...
int KLCTOOL_ShowNameMap()
{
    ...
    return NetLink_Do(_klctool_map_show, cmd, NULL, 0);
}

int KLCTOOL_ShowEvob()
{
    int cmd = KLC_NL_TYPE_CMD(KLC_NL_TYPE_EVENT, KLC_NL_EVOB_GET);
    return NetLink_Do(_klctool_evob_show, cmd, NULL, 0);
}
